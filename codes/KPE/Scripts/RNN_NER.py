...
# cross validaition using sklearn
from sklearn.model_selection import train_test_split

'''
    Train a RNN on the the dev corpus
...
train_x_pad = pad_sequences(train_x, maxlen=ngram)
print('Padding test data a...')
testa_x_pad = pad_sequences(testa_x, maxlen=ngram)
...
print('Build model...')
model = Sequential()
model.add(Embedding(vocab_size, embedding_size, input_length=ngram))
...
# train model
print('Train...')
model.fit(train_x_pad, train_y_categorical,
            batch_size=batch_size,
            epochs=epochs,
            validation_data=(testa_x_pad, testa_y_categorical))

# test model
print('Test...')
testa_y_predict = model.predict_classes(testa_x_pad, batch_size=batch_size)
# testb_y_predict = model.predict_classes(testb_x_pad, batch_size=batch_size)
